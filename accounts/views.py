...
class CurrentUserView(APIView):
    ...
    def get(self, request):
        ...
        if user.is_authenticated:
            ...
            # اضافه کردن اطلاعات داشبورد
            data = serializer.data
            data['dashboard_type'] = self.get_dashboard_type(user.role)
            data['redirect_url'] = self.get_redirect_url(user.role)
    ...
    def get(self, request):
        if request.user.is_authenticated:
            return Response({
                ...
                'dashboard_type': self.get_dashboard_type(request.user.role),
                'redirect_url': self.get_redirect_url(request.user.role)
            }, status=status.HTTP_200_OK)
